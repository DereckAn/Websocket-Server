...
export async function getSquareOrderById(
  orderId: string
  ...
  try {
    ...
    if (response.order) {
      const order = JSON.parse(
        JSON.stringify(response.order, handleBigIntSerialization)
      );
...
export async function processSquareWebhook(
    ...
    if (["order.created", "order.updated"].includes(event.type)) {
      ...
      try {
        ...
        if (order && isValidOrder(order)) {
          ...
          log(
            "info",
            `Order ${orderId} processed successfully - COMPLETED state`
          );
        } else if (order) {
          log(
            "info",
            `Order ${orderId} skipped - state: ${order.state} (not COMPLETED)`
          );
        }
      } catch (error) {
  ...
  log(
    "info",
    `Webhook processing complete. ${processedOrders.length} orders ready for broadcast`
  );
